...
public class ItemService {
    ...
    public static void addIngredientToItem(int itemId, int ingredientId) {
        // Önce ürün ve madde var mı kontrol et
    ...
    // Bir maddenin alt maddelerini listeleme
    public static void listSubIngredients(int ingredientId) {
        ...
        // Tüm ürün ve maddelerin isimlerini döndür
    ...
    public static List<String> getIngredientsForItem(int productId) {
        String sql = "SELECT i.name, i.price FROM items i INNER JOIN item_ingredients ii ON i.id = ii.ingredient_id WHERE ii.item_id = ?";
        ...
        try (Connection conn = DatabaseManager.connect();
            ...
            while (rs.next()) {
                String ingredient = rs.getString("name") + " - Fiyat: " + rs.getDouble("price");
